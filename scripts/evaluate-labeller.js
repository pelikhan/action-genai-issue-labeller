...
/**
 * Evaluation script for the GenAI Issue Labeller
 * 
 * This script fetches the last 100 issues from a repository,
 * simulates the labelling process, and evaluates accuracy against existing labels.
 */

const https = require('https');
const fs = require('fs');

class LabellerEvaluator {
  ...
  /**
   * Simulate the AI labelling process
   * Since we can't run the actual AI model easily, this is a simplified simulation
   * that would need to be replaced with actual AI calls in a real implementation
   */
  simulateLabelSuggestion(issue, availableLabels) {
    // This is a simplified heuristic-based approach for demonstration
    ...
    // Simple keyword-based labelling (this is just for demo purposes)
    const labelKeywords = {
      'bug': ['bug', 'error', 'issue', 'problem', 'fail', 'broken', 'crash'],
      'enhancement': ['feature', 'enhance', 'improve', 'add', 'support'],
      'documentation': ['doc', 'readme', 'document', 'guide', 'manual'],
      'question': ['question', '?', 'how', 'help', 'unclear'],
      'good first issue': ['good first issue', 'beginner', 'easy', 'starter'],
      'help wanted': ['help wanted', 'help', 'volunteer'],
      'invalid': ['invalid', 'spam', 'off-topic'],
      'wontfix': ['wontfix', "won't fix", 'rejected'],
      'duplicate': ['duplicate', 'dup', 'already', 'same as']
    };
    ...
    for (const [labelName, keywords] of Object.entries(labelKeywords)) {
      ...
      if (labelExists && keywords.some(keyword => content.includes(keyword))) {
        suggestedLabels.push(labelName);
      }
    }

    // If no labels matched, try to match by similarity to available labels
    if (suggestedLabels.length === 0) {
      for (const label of availableLabels) {
        if (content.includes(label.name.toLowerCase())) {
    ...
    return suggestedLabels.slice(0, 3); // Limit to 3 labels
  }
  ...
  async evaluate(issueCount = 100) {
    console.log(`🚀 Starting evaluation for ${this.owner}/${this.repo}`);
    ...
    try {
      ...
      for (let i = 0; i < issues.length; i++) {
        ...
        // Simulate AI labelling
        const suggestedLabels = this.simulateLabelSuggestion(issue, labels);
...
async function main() {
  ...
  if (args.length < 2 || args.includes('--help') || args.includes('-h')) {
    console.log(`
Usage: node evaluate-labeller.js <owner> <repo> [options]
...
Examples:
  node evaluate-labeller.js microsoft vscode --count 50
  node evaluate-labeller.js octocat Hello-World
  GITHUB_TOKEN=your_token node evaluate-labeller.js owner repo
...
`);
  ...
  try {
    const evaluator = new LabellerEvaluator(owner, repo, token);
...
module.exports = { LabellerEvaluator };